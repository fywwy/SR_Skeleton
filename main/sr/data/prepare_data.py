...
def resize_and_convert(img, size, resample):
    if(img.size[0] != size):
        img = trans_fn.resize(img, size, resample)  # 调整图片分辨率到指定分辨率大小
...
def resize_multiple(img, sizes=(16, 128), resample=Image.BICUBIC, lmdb_save=False):
    ...
    lr_img = resize_and_convert(img, sizes[0], resample) # sizes[0], 就变为目标尺寸了
...
def resize_worker(img_file, sizes, resample, lmdb_save=False):  # 此处sizes的值还是列表[16, 128]
...
if __name__ == '__main__':
    ...
    #                     default='{}/dataset/test3'.format(Path.home()))
    parser.add_argument('--path', '-p', type=str,
                        default='../dataset/test3')
    ...
    parser.add_argument('--resample', type=str, default='bicubic')      # 指定重采样方法
